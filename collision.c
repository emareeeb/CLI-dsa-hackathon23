...
// Function to check for collisions and terminate the program if detected
void checkForCollisionsAndTerminate(Aircraft* newAircraft, AirTrafficControlSystem* atcSystem) {
    for (int i = 0; i < atcSystem->priority_queue.size; ++i) {
        Aircraft existingAircraft = atcSystem->priority_queue.heap[i].aircraft;
        if (newAircraft->latitude == existingAircraft.latitude && newAircraft->longitude == existingAircraft.longitude && existingAircraft.altitude==newAircraft->altitude) {
            printf("COLLISION detected between Aircraft %d and Aircraft %d. Program terminated.\n",
                   newAircraft->aircraft_id, existingAircraft.aircraft_id);
            exit(EXIT_FAILURE);
        }
    }
}
...
void addAircraft(AirTrafficControlSystem* atcSystem, int aircraft_id, double latitude, double longitude, double altitude, double speed, double dest_latitude, double dest_longitude) {
    ...
    // Check for collisions before adding the new aircraft
    checkForCollisionsAndTerminate(&newAircraft, atcSystem);
